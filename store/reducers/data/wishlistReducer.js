...
const slice = createSlice({
  ...
  reducers: {
    ...
    fetchRecomendListSuccess(state, action){
      state.recomendList = action.payload;
      state.success = null;
    },
    // UPDATE WISHLIST
...
export function fetchWishList(queryParams) {
  return async () => {
    ...
    try {
      ...
      const userId = '1313'; // Replace with the actual user ID
      const responsee = await axiosMLService.get('ads/my', {
      params: {
        user_id: 1313,
      },
    });
 ...
 * @returns 
 */
export function fetchRecomendList(queryParams) {
  return async () => {
    dispatch(slice.actions.startLoading());
    ...
    try {
      ...
      const responsee = await axiosMLService.get(`ads/ads/similar/${queryParams}`);
      const formattedAds = responsee.data.map(transformAdData);
      console.log(formattedAds);
      console.log(responsee)
      // const responses = await axiosServices.get(`ads/advertisement/${1}`);
      ...
      dispatch(slice.actions.fetchRecomendListSuccess(formattedAds));
    } catch (error) {
      dispatch(slice.actions.hasError(error));
    } finally {
      dispatch(slice.actions.finishLoading());
